...
const Capsule = {
    ...
    pick: (all, taste, opts) => {
        ...
        // мягкий добор: если честный отбор недобрал до CAP, ослабляем пороги
        // рейтинга/голосов и добираем из того же пула all — набор 100 карточек
        // теперь получается ощутимо чаще, полная страховка (сброс жанров/тем)
        // остаётся только на случай, когда пришло совсем мало кандидатов.
        if (final.length < CAP && all.length) {
            const beforeSoft = final.length;
            const softVote = relax ? 5.0 : 5.3;
            const softCount = relax ? 15 : 25;
...
const Search = {
    ...
    run: (query, taste, cb, force, depth) => {
        ...
        const stage2 = (kwIds, people) => {
            ...
            const discover = (media, page, extra) => {
                // на первом проходе пороги чуть мягче собственных ctx.minVote/minVotes —
                // это уменьшает шанс пустой/скудной выдачи по узким тематическим запросам,
                // не давая при этом совсем случайным карточкам попасть в топ (Search.rank
                // всё равно довзвешивает по совпадению жанра/названия).
                const p = {
                    sort_by: force ? pickOne(['popularity.desc', 'vote_average.desc', 'vote_count.desc']) : 'popularity.desc',
                    include_adult: false, page,
                    'vote_count.gte': Math.max(10, Math.round(ctx.minVotes * 0.7)),
                    'vote_average.gte': Math.max(0, ctx.minVote - 0.4)
                };
            ...
            parallel(tasks, (packs) => {
                ...
                // авто-ослабление: пороги вниз, только текстовый поиск
    ...
    relax: (query, ctx, taste, force, cb) => {
        ...
        const tasks = [];
        // расширенный набор страниц/жанров при авто-ослаблении — пустая выдача
        // становится ещё менее вероятной, а набор ближе к полному CAP
        [1, 2, 3].forEach(page => {
    ...
    rank: (list, ctx, taste, force) => {
        ...
        list.forEach(it => {
            ...
            let s = 0;
            if (exact && (title === exact || orig === exact)) s += 30;
            else if (exact && (title.indexOf(exact) === 0 || orig.indexOf(exact) === 0)) s += 14;
            else if (exact && (title.indexOf(exact) > -1 || orig.indexOf(exact) > -1)) s += 7;
            stems.forEach(st => {
                if (!st) return;
                if (title === st) s += 10;
            ...
            // если запрошены жанры, но карточка не попала ни в один — штраф,
            // чтобы «без ужасов» / конкретный жанр не размывались случайными совпадениями
        ...
        if (force && top.length > 6) {
            const block = 10, res = [];
            // первую (самую точную) карточку не перемешиваем
            res.push(top[0]);
...
.menu__item[data-action="capsule_mod_entry"] .menu__ico svg{transition:filter .15s ease}
.menu__item[data-action="capsule_mod_entry"].focus .menu__ico svg,
.menu__item[data-action="capsule_mod_entry"]:hover .menu__ico svg{
mix-blend-mode:difference;filter:invert(1) grayscale(1) contrast(1.35);
}`;
...
const UI = {
    ...
    stageOffset: () => {
        ...
        // больше готовых пунктов: от -3 до +3 с шагом 0.5
        const presets = [];
