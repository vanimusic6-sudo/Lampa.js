...
skip: (key, n, span) => { span = Math.max(1, span | 0); const all = pGet('pcur', {}) || {}; all[key] = (((all[key] | 0) + n) % span); pSet('pcur', all); },
reset: () => pSet('pcur', {})
};
// ═══════════════════════════════════════════ СЛОВАРИ
...
if (s === 3) {
wrap.appendChild(el('div', 'cm-onb-title', 'Настроения на сегодняшний вечер?'));
...
'vote_count.gte': force ? 50 + rnd(100) : 100,
'vote_average.gte': clamp(taste.avgVote ? taste.avgVote - 0.8 : 5.8, 5.5, 7.6),
include_adult: false
}, 'movie', 'genre', null, pagesOf('gm', force ? 8 : 6));
add('/discover/tv', {
with_genres: gt, sort_by: sortFor('gt'),
'vote_count.gte': 50, 'vote_average.gte': 5.8, include_adult: false
}, 'tv', 'genre', null, pagesOf('gt', force ? 6 : 4));
}
if (topK.length) {
...
if (picked.length < Math.min(20, CAP) && topG.length) {
...
const beforeSoft = final.length;
const softVote = relax ? 4.5 : 5.0;
const softCount = relax ? 10 : 15;
const already = new Set();
final.forEach(it => already.add(it.media_type + '_' + it.id));
const soft = [];
all.forEach(it => {
const key = it.media_type + '_' + it.id;
if (already.has(key)) return;
if (!it.poster_path) return;
if (!it.vote_average || it.vote_average < softVote) return;
if ((it.vote_count || 0) < softCount) return;
if (it.id && taste.watched && taste.watched[it.id]) return;
const ago = Seen.ago(it.id);
if (ago === 1) return;
already.add(key);
it._score = (it.vote_average || 0) + clamp((it.vote_count || 0) / 4000, 0, 1);
soft.push(it);
});
soft.sort((a, b) => b._score - a._score);
for (let i = 0; i < soft.length && final.length < CAP; i++) take(soft[i]);
if (final.length > beforeSoft) Log.push('ОТБОР: мягкий добор дал ещё ' + (final.length - beforeSoft) + ' карточек');
}
...
// человек: «с Томом Хэнксом», «режиссёр Нолан» или просто имя без жанров/тегов
...
const pages = Cursor.take(ckey, depthScale(force ? 8 : 6), SPAN);
...
'vote_count.gte': Math.max(5, Math.round(ctx.minVotes * 0.5)),
'vote_average.gte': Math.max(0, ctx.minVote - 0.6)
};
...
}, (d) => done(markList(d && d.results, 'movie', 'person', { person: p.name, query })), () => done([]), { force }));
});
tasks.push(done => Net.get('/person/' + p.id + '/combined_credits', {}, (d) => {
const cast = ((d && d.cast) || []).concat((d && d.crew) || []);
done(markList(cast.slice(0, 40), null, 'person', { person: p.name, query }));
}, () => done([]), { ttl: 604800000 }));
});
// прямой поиск по названию
if (ctx.tokens.length) {
tasks.push(done => Net.get('/search/multi', { query: String(query).slice(0, 70), page: 1, include_adult: false }, (d) => done(markList(d && d.results, null, 'search', { query })), () => done([]), { force }));
if (ctx.exact) {
tasks.push(done => Net.get('/search/movie', { query: String(query).slice(0, 70), page: 1, include_adult: false }, (d) => done(markList(d && d.results, 'movie', 'title', { query })), () => done([]), { force }));
tasks.push(done => Net.get('/search/tv', { query: String(query).slice(0, 70), page: 1, include_adult: false }, (d) => done(markList(d && d.results, 'tv', 'title', { query })), () => done([]), { force }));
}
}
...
if (ranked.length >= 6) return fire(ranked, ctx);
...
if (ctx.genresM.length) [1, 2].forEach(page => tasks.push(done => Net.get('/discover/movie', {
with_genres: ctx.genresM.slice(0, 2).join(','), sort_by: 'popularity.desc',
include_adult: false, page, 'vote_count.gte': 15
}, (d) => done(markList(d && d.results, 'movie', 'search', { query })), () => done([]), {})));
if (ctx.genresT.length) tasks.push(done => Net.get('/discover/tv', {
with_genres: ctx.genresT.slice(0, 2).join(','), sort_by: 'popularity.desc',
include_adult: false, page: 1, 'vote_count.gte': 15
}, (d) => done(markList(d && d.results, 'tv', 'search', { query })), () => done([]), {}));
if (ctx.tags.length) tasks.push(done => Net.get('/discover/movie', {
sort_by: 'popularity.desc', include_adult: false, page: 1, 'vote_count.gte': 10
}, (d) => done(markList(d && d.results, 'movie', 'search', { query })), () => done([]), {}));
parallel(tasks, (packs) => {
const list = [];
(packs || []).forEach(p => { if (isArr(p)) list.push.apply(list, p); });
...
if (ctx.minVote && (it.vote_average || 0) < ctx.minVote - 0.8) return;
const title = norm(it.title || it.name);
const orig = norm(it.original_title);
const over = norm(it.overview);
let s = 0;
if (exact && (title === exact || orig === exact)) s += 50;
else if (exact && (title.indexOf(exact) === 0 || orig.indexOf(exact) === 0)) s += 20;
else if (exact && (title.indexOf(exact) > -1 || orig.indexOf(exact) > -1)) s += 10;
stems.forEach(st => {
if (!st) return;
if (title === st) s += 10;
...
let wantedHits = 0;
(it.genre_ids || []).forEach(gid => { if (wanted.indexOf(gid) > -1) { s += 4; wantedHits++; } const g = TV2MOVIE[gid] || gid; if (gWeight[g]) s += 2 * (gWeight[g] / maxG); });
if (wanted.length && !wantedHits) s -= 3;
if (it._src === 'title') s += 4;
if (it._src === 'person') s += 5;
...
// значение хранится как число em (строкой в pGet, парсится при использовании);
// шаг ◀ ▶ в UI.stageOffset двигает именно эту величину на ±0.1em
const offsetNum = () => { const v = parseFloat(String(pGet('stage_offset', '0em')).replace('em', '')); return isFinite(v) ? v : 0; };
const offsetLabel = (val) => { const n = typeof val === 'number' ? val : parseFloat(String(val).replace('em', '')) || 0; if (!n) return 'по умолчанию'; return (n > 0 ? '+' : '') + n.toFixed(1) + 'em'; };
// ═══════════════════════════════════════════ CSS
const CSS = `
...
.cm-chip.cm-static{justify-content:center;font-weight:800;cursor:default;background:rgba(255,255,255,.02)}
...
/* ── ЛОГОТИП В МЕНЮ LAMPA: инверсия под цвет фона под ним (только в фокусе) ── */
.menu__item[data-action="capsule_mod_entry"] .menu__ico svg {
    transition: fill .2s ease, color .2s ease;
}
.menu__item[data-action="capsule_mod_entry"].focus .menu__ico svg,
.menu__item[data-action="capsule_mod_entry"]:hover .menu__ico svg {
    mix-blend-mode: difference;
}
.menu__item[data-action="capsule_mod_entry"].cm-light-bg.focus .menu__ico svg,
.menu__item[data-action="capsule_mod_entry"].cm-light-bg:hover .menu__ico svg {
    fill: #000 !important;
    color: #000 !important;
    mix-blend-mode: normal;
    filter: none;
}
.menu__item[data-action="capsule_mod_entry"]:not(.cm-light-bg).focus .menu__ico svg,
.menu__item[data-action="capsule_mod_entry"]:not(.cm-light-bg):hover .menu__ico svg {
    fill: #fff !important;
    color: #fff !important;
    mix-blend-mode: normal;
    filter: none;
...
opts.chips.forEach(ch => { const c = el('div', 'cm-chip' + (ch.noClose ? ' cm-static' : ''), esc(ch.label)); c._cmAction = () => { if (ch.noClose) { if (ch.onSelect) ch.onSelect(); return; } Modal.close(true); if (ch.onSelect) ch.onSelect(); }; wrap.appendChild(c); nodes.push(c); });
...
b._cmAction = () => { Modal.close(!!it.onSelect); if (it.onSelect) it.onSelect(); };
box.appendChild(b); nodes.push(b);
});
ov.appendChild(box);
document.body.appendChild(ov);
ov.onclick = (e) => { if (e.target === ov) Modal.close(); };
Modal.st = { ov, box, nodes, idx: 0, gridLen, tag: opts.tag || '' };
...
if (st.tag === 'stage_offset' && (dir === 'up' || dir === 'down')) {
    const cur = offsetNum();
    const STEP = 0.2, MIN = -6, MAX = 6;
    const newVal = clamp(cur + (dir === 'down' ? STEP : -STEP), MIN, MAX);
    if (Math.abs(newVal - cur) > 0.01) {
        pSet('stage_offset', newVal + 'em');
        StageOffset.apply(View.root);
        const titleEl = st.box.querySelector('h3');
        if (titleEl) titleEl.textContent = 'Смещение интерфейса (' + offsetLabel(newVal) + ')';
        if (st.nodes[1]) st.nodes[1].textContent = offsetLabel(newVal);
        const presets = [];
        for (let v = MIN; v <= MAX; v += 0.5) presets.push(Math.round(v * 10) / 10);
        let itemIdx = st.gridLen;
        presets.forEach(v => {
            const node = st.nodes[itemIdx];
            if (node) {
                const isSelected = Math.abs(newVal - v) < 0.01;
                node.innerHTML = esc((isSelected ? '● ' : '○ ') + offsetLabel(v));
            }
            itemIdx++;
        });
    }
...
const cur = offsetNum();
const setVal = (n) => {
const v = clamp(Math.round(n * 10) / 10, MIN, MAX);
pSet('stage_offset', v + 'em');
StageOffset.apply(View.root);
UI.stageOffset();
};
const presets = [];
for (let v = MIN; v <= MAX; v += 0.5) presets.push(Math.round(v * 10) / 10);
Modal.open({
title: 'Смещение интерфейса (' + offsetLabel(cur) + ')',
tag: 'stage_offset',
chips: [
{ label: '◀ −' + STEP.toFixed(1), onSelect: () => setVal(cur - STEP) },
{ label: offsetLabel(cur), noClose: true },
{ label: '+' + STEP.toFixed(1) + ' ▶', onSelect: () => setVal(cur + STEP) }
],
items: presets.map(v => ({ label: (Math.abs(cur - v) < 0.01 ? '● ' : '○ ') + offsetLabel(v), onSelect: () => setVal(v) }))
.concat([{ label: 'Сброс (по умолчанию)', onSelect: () => setVal(0) }, { label: 'Назад', onSelect: () => UI.settings() }])
});
},
themes: () => {
...
const fixMenuLogo = () => {
    const item = document.querySelector('.menu__item[data-action="capsule_mod_entry"]');
    if (!item) return;
    let el = item;
    let bgColor = null;
    ...
    const match = bgColor ? bgColor.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/) : null;
    if (match) {
        const r = parseInt(match[1]), g = parseInt(match[2]), b = parseInt(match[3]);
        const luminance = (0.299 * r + 0.587 * g + 0.114 * b) / 255;
        if (luminance > 0.5) item.classList.add('cm-light-bg');
        else item.classList.remove('cm-light-bg');
    }
};
setInterval(fixMenuLogo, 1500);
